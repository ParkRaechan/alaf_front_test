...
import { ArrowLeft, LogOut, User, ShieldCheck } from 'lucide-react';
    ...
    <div className="pc-container">
      ...
      <main className="pc-main">
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
            ...
            {/* ----------------------------------------------------------- */}
            {/* 🚨 관리자 전용 메뉴 (user.role이 'ADMIN'일 때만 렌더링) */}
            {/* ----------------------------------------------------------- */}
            {user && user.role === 'ADMIN' && (
                <div style={{ marginTop: 40 }}>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#2b8a3e', fontSize: 20 }}>
                        <ShieldCheck size={24} /> 관리자 전용 메뉴
                    </h3>
                    
                    <div 
                        onClick={() => navigate('/admin/requests')} // 🚨 관리자 페이지 라우팅 주소
                        style={{ 
                            background: '#ebfbee', // 연한 초록색 배경
                            padding: '25px 30px', 
                            borderRadius: 16, 
                            marginTop: 15, 
                            cursor: 'pointer',
                    ...
                    >
                        <div>
                            ...
                            <p style={{ margin: '8px 0 0', color: '#51cf66', fontSize: 14 }}>
                                사용자들이 제출한 증거를 확인하고 승인 및 거절을 처리합니다.
                            </p>
                        </div>
                        <div style={{ background: '#2b8a3e', color: 'white', padding: '10px 20px', borderRadius: 8, fontWeight: 'bold' }}>
                            이동하기
                        </div>
                    </div>
                </div>
